...
Although `parallel_scheduler` [@P2079R10] was a long time in the making, getting it adopted in Sofia 2025 still left the impression that some of the fine details were not sufficiently discussed. This paper proposes to iterate on some of these aspects, aiming to achieve the best possible outcome from `parallel_scheduler`.
...
- It is a property that can be queried from the receiver, yet we don't use the API (non-uniform behavior).
...
- Complicates storage, and may lead to larger storage needs in the operation state.
...
The authors don't see any changes in this area that would benefit users in a tangible way.
...
We aim to build on top of [@D3826].


## Removing customization mechanisms

Eric Niebler announced on the reflector that NVIDIA added an NB comment suggesting removing customization mechanisms from senders/receivers from C++26, and adding them in future versions of the standard.
This was translated into [@D3826].
If that change is to be adopted, it also has implications for `parallel_scheduler`.
We explore here the implications.

Although originally the thought was to completely remove customizations, after discussions the consensus was for [@D3826] to move toward a model that allows controlled customizations.
This would allow vendors to provide customizations of `bulk` for `parallel_scheduler`.
But, this mechanism is not widely exposed to users.
...
[The following changes also contain the changes from D3826:]{.ednote}
