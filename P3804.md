...
- [10.1]{.pnum} If [`sndr`]{.rm} [`child`]{.add} completes with values `vals`, let `args` be a pack of lvalue subexpressions designating `vals`, then `b.schedule_bulk_chunked(@[shape]{.rm} [parallelizable ? shape : 1]{.add}@, r, s)` is called, where:
...
- [11.1]{.pnum} If [`sndr`]{.rm} [`child`]{.add} completes with values `vals`, let `args` be a pack of lvalue subexpressions designating `vals`, then `b.schedule_bulk_unchunked(@[shape]{.rm} [parallelizable ? shape : 1]{.add}@, r, s)` is called, where:
