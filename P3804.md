...
This means that, on in the worst case we would use two stop tokens (in the best one, we are using only one).
...
- Increases API space (we need another backend class with a method dedicated to requesting stop).
- It is a property that can be queries from the receiver, yet we don't use the API (non-uniform behavior).
- If we can't query stop tokens through `try_query` interface, for C++26 we won't have any properties that would use `try_query`.
- Complicates storage, and may lead to larger storage needs in operation state.
...
- the frontend needs to store a pointer of the returned `backend_operation` object, so that is another pointer.
