...
First, on the user-facing code, we do not need to specify that `parallel_scheduler` needs to be customized.

Secondly, from the backend we need to remove support for `bulk`.
This means:

* `schedule_bulk_chunked` and `schedule_bulk_unchunked` methods from `parallel_scheduler_backend`
* `bulk_item_receiver_proxy` struct
...
[If sender customization is not removed, then add the following:]{.ednote}
...
[If customizations are removed from `std::execution`, then also apply the following changes in _ Header `<execution>` synopsis_ [execution.syn]:]{.ednote}

```cpp
// [exec.sysctxrepl], namespace system_context_replaceability
namespace std::execution::system_context_replaceability {
  struct receiver_proxy;
  @[`struct bulk_item_receiver_proxy;`]{.rm}@
...
[If customizations are removed from `std::execution`, then also apply the following changes in _`query_parallel_scheduler_backend`_ [exec.sysctxrepl.query]:]{.ednote}

```cpp
namespace std::execution::system_context_replaceability {
  struct receiver_proxy {
    @[not repeating the body of `receiver_proxy`]{.ednote}@
  };

  @[`struct bulk_item_receiver_proxy : receiver_proxy {`]{.rm}@
    @[`virtual void execute(size_t, size_t) noexcept = 0;`]{.rm}@
  @[`};`]{.rm}@
}
```

[4]{.pnum} `receiver_proxy` represents a receiver that will be notified by the implementations of `parallel_scheduler_backend` to trigger the completion operations. [`bulk_item_receiver_proxy` is derived from `receiver_proxy` and is used for `bulk_chunked` and `bulk_unchunked` customizations that will also receive notifications from implementations of `parallel_scheduler_backend` corresponding to different iterations.]{.rm}

<hr>

[If customizations are removed from `std::execution`, then also apply the following changes in _Class `parallel_scheduler_backend`_ [exec.sysctxrepl.psb]:]{.ednote}
...
namespace std::execution::system_context_replaceability {
  struct parallel_scheduler_backend {
    ...
    @[`virtual void schedule_bulk_chunked(size_t, bulk_item_receiver_proxy&,`]{.rm}@
                                       @[`span<byte>) noexcept = 0;`]{.rm}@
    @[`virtual void schedule_bulk_unchunked(size_t, bulk_item_receiver_proxy&,`]{.rm}@
                                         @[`span<byte>) noexcept = 0;`]{.rm}@
  };
}
```

[Also remove descriptions for `schedule_bulk_chunked` and `schedule_bulk_unchunked`, with paragraphs 4-9]{.ednote}
